...
void Mesh::setLocation(GLuint location, std::size_t elementCount, std::size_t offsetCount, std::size_t strideCount)
{
    glBindVertexArray(this->m_vao);
    glBindBuffer(GL_ARRAY_BUFFER, this->m_vbo);
    glEnableVertexAttribArray(location);

    glVertexAttribPointer(location, static_cast<GLint>(elementCount), GL_FLOAT, GL_FALSE,
